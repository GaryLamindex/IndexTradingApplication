        ...
        if self.check_exec(timestamp, freq="Daily", relative_delta=1):
            ...
            if self.loop == 0:
                ...
                capital_for_each_stock = float(self.account_snapshot.get(
                    "TotalCashValue")) / self.number_of_stocks  # or should use buying power ???
                ...
                for ticker in temp_ticker:
                    ...
                    share_purchase = math.floor(capital_for_each_stock / ticker_price) * 3  # 3x leverage
                    action_msg = self.trade_agent.place_buy_stock_limit_order(ticker, share_purchase, ticker_price * (
                            1 + self.acceptance_range), timestamp)
                ...
                if self.check_max_drawdown_control_operation(timestamp):
                    ...
                    for ticker in temp_ticker:
                        ...
                        # this statement will always true since no self.check_fuse_liquidation
                        if self.reg_exec[ticker] == False:
                            target_ex_liq = self.rebalance_margin * float(
                                self.account_snapshot.get("GrossPositionValue"))
                            ...
                            if self.max_drawdown_dodge[ticker]:  # the stock is dodged
                                ...
                                if self.check_buy_back(ticker, ticker_price):
                                    self.buy_back_position(ticker, ticker_price, buying_power,
                                                           timestamp)  # the function handled writing of action message
                                else:
                                ...
                                if self.check_max_drawdown_dodge(ticker, ticker_price):  # needs to dodge
                                    self.liquidate_stock_position(ticker, ticker_price * (1 - self.acceptance_range),
                                                                  timestamp)
                                    ...
                                    if float(self.account_snapshot.get("ExcessLiquidity")) > target_ex_liq:
                                        ...
                                        if target_share_purchase > 0:
                                            action_msg = self.trade_agent.place_buy_stock_limit_order(ticker,
                                                                                                      target_share_purchase,
                                                                                                      ticker_price * (
                                                                                                              1 + self.acceptance_range),
                                                                                                      timestamp)
                                            self.action_msgs.append(action_msg)
                    ...
                    if float(self.account_snapshot.get("ExcessLiquidity")) < main_ex_liq:
                        ...
                        if target_share_sell > 0:
                            action_msg = self.trade_agent.place_sell_stock_limit_order(ticker, target_share_sell,
                                                                                       ticker_price * (
                                                                                               1 + self.acceptance_range),
                                                                                       timestamp)
                            self.action_msgs.append(action_msg)
    ...
    def check_fuse_liquidation(self, timestamp, realtime_stock_data_dict):
        ...
        for ticker in self.tickers:
            ...
            if realtime_stock_data_dict[ticker]['last'] > self.benchmark_drawdown_price[ticker] * 0.5:
                self.liquidate_stock_position(ticker,
                                              realtime_stock_data_dict[ticker]['last'] * (1 + self.acceptance_range),
                                              timestamp)
