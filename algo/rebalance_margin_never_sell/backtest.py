...
from pythonProject.algo.rebalance_margin_never_sell.algorithm import rebalance_margin_never_sell
from pythonProject.algo.rebalance_margin_wif_max_drawdown_control.algorithm import rebalance_margin_wif_max_drawdown
...
from pythonProject.engine.aws_engine.dynamo_db_engine import dynamo_db_engine
...
class backtest(object):
    ...
    def __init__(self, tickers, initial_amount, start_date, end_date, cal_stat, rabalance_dict, maintain_dict, purchase_exliq_ratio_dict, data_freq, user_id, db_mode):
        ...
        self.table_info = {"mode": "backtest", "strategy_name": "rebalance_margin_never_sell", "user_id": user_id}
        ...
        for rebalance in range(rebalance_start, rebalance_end, rebalance_step):
            for maintain in range(maintain_start, maintain_end, maintain_step):
                for purchase_exliq_ratio in range(purchase_exliq_ratio_start, purchase_exliq_ratio_end, purchase_exliq_ratio_step):
                    ...
                    backtest_spec = {"rebalance_margin":rebalance_margin, "purchase_exliq":purchase_exliq}
                    spec_str = ""
                    ...
                    algorithm = rebalance_margin_never_sell(trade_agent, portfolio_data_engine, self.tickers, acceptance_range, rebalance_margin)
