...
class ibkr_acc_data(object):
    ...
    def update_portfolio_item(self, ticker, position, marketPrice, averageCost, marketValue, realizedPNL, unrealizedPNL, initMarginReq, maintMarginReq, costBasis):
        ...
        print(f"{ticker} portfolio dict: {updating_portfolio_dict}")
        # """now is hard coded !"""
        # if len(self.portfolio) == 2: # intially fully filled
        #     self.portfolio = self.portfolio[1:] # get rid of the first one
        # self.portfolio.append(updating_portfolio_dict)

        # create a temp dict, since we need to drop the ['ticker'] item in the dictionary

        for item in self.portfolio:
            if item.get("ticker") == ticker:
                item.update((k,v) for k,v in updating_portfolio_dict.items() if v is not None)
                return
        # if not found (i.e. not returned)
        self.portfolio.append(updating_portfolio_dict)
    ...
    def get_account_snapshot(self):
        ...
        account_snapshot.update(self.trading_funds)
        account_snapshot.update(self.mkt_value)
