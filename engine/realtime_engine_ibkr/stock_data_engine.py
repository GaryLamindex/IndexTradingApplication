...
class ibkr_stock_data_io_engine:
    ...
    def __init__(self,ib_instance):
        ...
        self.output_filepath = str(pathlib.Path(__file__).parent.parent.parent.parent.parent.resolve()) + "/ticker_data/one_min"
    ...
    def get_ibkr_open_price(self,tickers):
        ...
        for ticker in tickers:
            ...
            while math.isnan(raw_ticker_info.bid) or math.isnan(raw_ticker_info.bidSize) or math.isnan(raw_ticker_info.ask) or math.isnan(raw_ticker_info.askSize) or math.isnan(raw_ticker_info.last) or math.isnan(raw_ticker_info.lastSize) or math.isnan(raw_ticker_info.volume):
            ...
            ticker_info = {"timestamp":raw_ticker_info.time.timestamp(),"bid":raw_ticker_info.bid,"bidSize":raw_ticker_info.bidSize,"ask":raw_ticker_info.ask,"askSize":raw_ticker_info.askSize,"last":raw_ticker_info.last,"lastSize":raw_ticker_info.lastSize,"volume":raw_ticker_info.volume,"close":raw_ticker_info.close,"halted":raw_ticker_info.halted}
    ...
    def get_historical_data_by_range(self,ticker,start_timestamp,end_timestamp,bar_size,regular_trading_hour):
        ...
        while (current_end_timestamp > start_timestamp):
            ...
            current_data_df['timestamp'] = current_data_df[['date']].apply(lambda x: x[0].replace(tzinfo=dt.timezone(dt.timedelta(hours=8))).timestamp(), axis=1).astype(int)
            # write to csv
    ...
    def get_multiple_historical_data_by_range(self,tickers,start_timestamp,end_timestamp,bar_size,regular_trading_hour):
        for ticker in tickers:
