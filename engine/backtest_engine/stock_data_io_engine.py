...
class crypto_local_engine:
    ...
    def get_field_by_timestamp(self, timestamp, field):
        return self.full_ticker_df.loc[self.full_ticker_df['timestamp'] == timestamp, field].item()

    def has_started_at_timestamp(self, timestamp):
        return self.full_ticker_df.loc[0, 'timestamp'] <= timestamp
    ...
    def get_pct_change_by_timestamp(self, periods, timestamp):
        ...
        return df.loc[df['timestamp'] == timestamp, 'pct_change'].item()
