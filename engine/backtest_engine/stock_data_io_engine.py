...
class online_engine:
    ...
    def get_ticker_item_by_timestamp(self,ticker,timestamp):
        ...
        query_result = self.db_engine.query_all_by_range(f"his_data_{self.freq}",ticker,"timestamp",[timestamp,timestamp])
...
class local_engine:
    ...
    def get_n_days_data(self,timestamp,n): # the timestamp parameter MUST be an integer or a float
        ...
        while (count < n):
            ...
            lower_dt = dt.datetime.strptime(lower_datestring,"%Y-%m-%d %H:%M:%S").replace(tzinfo=dt.timezone(dt.timedelta(hours=-5)))
            upper_dt = dt.datetime.strptime(upper_datestring,"%Y-%m-%d %H:%M:%S").replace(tzinfo=dt.timezone(dt.timedelta(hours=-5)))

            selection_range = [float(lower_dt.timestamp()),float(upper_dt.timestamp())]

            temp_df = self.full_ticker_df[(self.full_ticker_df['timestamp'] >= selection_range[0]) & (self.full_ticker_df['timestamp'] <= selection_range[1])]
    ...
    def get_data_by_range(self,range):
        ...
        data_by_range_df = self.full_ticker_df[(self.full_ticker_df['timestamp'] >= float(range[0])) & (self.full_ticker_df['timestamp'] <= float(range[1]))].copy()
        data_by_range_df.sort_values(['timestamp'],ascending=True,kind='stable',inplace=True)
        data_by_range_df.reset_index(drop=True,inplace=True)
